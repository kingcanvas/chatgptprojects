...
Shardstead is an original 3D open-world building game in development for desktop and mobile browsers. The current playable milestone is **Trueframe (Build 007)**.
...
- Standalone iPhone Home Screen support with one measured visual/touch viewport, safe-area-aware landscape controls, aligned hitboxes, scrollable overlays, and an always-visible inventory close control
...
Trueframe is a major playable milestone, not the finished game. The larger design, technical roadmap, content plan, PC/mobile targets, optimization tiers, and production phases are documented in [PLANS.md](./PLANS.md).
